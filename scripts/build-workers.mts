...
const WORKER_TOOLS_PACKAGE = "@vercel-ship-2026-workers/worker-tools";
const WORKER_TOOLS_ENTRY = path.join(ROOT_DIR, "packages", "worker-tools", "src", "index.ts");
  ...
  return {
    ...
    setup(build: PluginBuild) {
      build.onResolve({ filter: /.*/ }, (args) => {
        if (args.path === WORKER_TOOLS_PACKAGE) {
          return { path: WORKER_TOOLS_ENTRY };
        }
